                                    ...
                                    cond_mean = NULL, sim_method = c("parametric", "bootstrap"), xreg = NULL, vreg = NULL, ...)
{
    ...
    if (burn > 0) {
        R <- R[-seq_len(burn), , , drop = FALSE]
        std_residuals <- std_residuals[, -seq_len(burn), , drop = FALSE]
    }
...
.dcc_constant_simulate_r <- function(object, nsim = 1, seed = NULL, h = 100, burn = 0, cond_mean = NULL, init_method = c("start", "end"), xreg = NULL, vreg = NULL, ...)
{
    ...
    std_residuals <- array(0, dim = c(nsim, h + burn, n_series))
    tmp_z <- matrix(rnorm((h + burn) * dim * nsim), ncol = dim, nrow = (h + burn) * nsim)
    ...
    for (i in 1:n_series) {
        std_residuals[,,i] <- matrix(tmp_z[,i], ncol = h + burn, nrow = nsim)
    }
    gsim <- NULL
    gsim <- future_lapply(1:n_series, function(i) {
...
.dcc_dynamic_predict_analytic <- function(object, h = 1, forc_dates = NULL, cond_mean = NULL, seed = NULL, newxreg = NULL, newvreg = NULL, ...)
{
    ...
    if (.has_arma_dynamics(object$spec$univariate) || .has_xreg_dynamics(object$spec$univariate)) {
