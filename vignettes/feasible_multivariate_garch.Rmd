...
# Covariance Decomposition Models {#sec-cd1}
...
dynamics may be separated, thus easing the estimation process. This decomposition
...
LL_{t}\left(\theta\right)&=\frac{1}{2}\left(\log\left(2\pi\right)+2\log\left|D_t\right|+\log\left|R\right|+z'_tR^{-1}z'_t\right)
\end{aligned}
\label{eq:2}
\end{equation}
...
\begin{equation}
\begin{aligned}
LL_t\left(\theta\right) &= \frac{1}{2}\left(\log\left(2\pi\right) + 2\log\left|D_t\right| + z`_t z_t\right) - \frac{1}{2}\left(z`_t z_t + \log\left|R_t\right| + z`_t R^{-1}_t z'_t\right)\\
...
\begin{equation}
\begin{aligned}
LL_t\left(\theta, \nu| \phi\right) & = \log\Gamma\left(\frac{\nu + k}{2}\right) - \log\Gamma\left(\frac{\nu}{2}\right) -\frac{k}{2}\log\left(\pi\left(\nu - 2\right)\right)\\ 
&- \frac{1}{2}\log\left|R_t\right| - \frac{\nu + 2}{2}\log\left(1 + \frac{1}{\nu-2}z_t' R_t^{-1} z_t\right)
\end{aligned}
\label{eq:9}
\end{equation}

where $\nu$ is the degrees of freedom parameter of the Student distribution, and $k$ is the number of series. 
...
\begin{equation}
\begin{aligned}
u_t & = C\left(F_1\left(z_{1,t}\right),\ldots,F_k\left(z_{k,t}\right)\right)\\
...
\begin{equation}
\begin{aligned}
LL_t\left(\theta\right) & = \sum\limits_{i=1}^{k} \left( -\frac{1}{2} \log h_{i,t} + \log f_{i,t}(z_{i,t}) \right) + \log c_t\left(u_{i,t},\ldots, u_{k,t}\right)\\
...
\begin{equation}
\log c_t\left(u_{1,t}, \ldots, u_{k,t}\right) = \log f_t\left( F_i^{-1}\left(u_{1,t}|\eta\right), \ldots, F_i^{-1}\left(u_{k,t}|\eta\right) | R_t,\eta \right) - \sum\limits_{i=1}^{k} \log f_i\left( F_i^{-1}(u_{i,t})|\eta \right)
\label{eq:12}
\end{equation}
...
subject to a common shape parameter of the multivariate Student Copula, and $f_t\left( F_i^{-1}\left(u_{1,t}|\eta\right), \ldots, F_i^{-1}\left(u_{k,t}|\eta\right) | R_t,\eta \right)$
...
The `tsmarch` package does not implement this specific model, but instead a scalar parameter is introduced to allow for asymmetric dynamics in the DCC model (**aDCC**).
...
Q_t & = \bar Q + \sum\limits_{j=1}^{q}{a_j\left(z_{t-j}z'_{t-j}-\bar Q\right)} +  \sum\limits_{j=1}^{p}{b_j\left(Q_{t-j}-\bar Q\right)}\\
R_t & = \text{diag}\left(Q_t\right)^{-1/2}Q_t\text{diag}\left(Q_t\right)^{-1/2}\\
z_t & = E \Lambda^{1/2} \epsilon_t
\end{aligned}
\label{eq:14}
\end{equation}

where $\epsilon_t$ is a vector of standard normal random variables, $\Lambda$ is the diagonal matrix of eigenvalues of $R_t$ and $E$ is the matrix of 
eigenvectors of $R_t$, such that:
...
\begin{equation}
z_t =  E \Lambda^{1/2} \epsilon_t \sqrt{\frac{\nu}{W}}
\label{eq:16}
\end{equation}
...
The GO-GARCH model in the `tsmarch` package is based on the approach described
...
selecting the first $m$ principal components of the residuals. In that case 
...
The estimation of the GO-GARCH model is performed in 3 steps.
...
The log-likelihood of the GO-GARCH model is composed of the individual log-likelihoods
...
In the presence of dimensionality reduction, $K$ is no longer square, we cannot 
...
using only univariate methods, providing for feasible estimation of large scale systems. 
The speed advantage of this approach does not come for free, as there is a corresponding
need for memory to handle the parallelization offered, particularly when generating
higher co-moment matrices.

## Co-Moments

The linear affine representation of the GO-GARCH model provides for a 
...
as in @Chen2007 or by simulation. The `tsmarch` package generates both simulated predictive
...
versions of the parameters $(\alpha_{i}, \beta_{i}, \delta_{i}, \mu_{i})$ as shown in \eqref{eq:35}. The density may be accurately
...
\begin{figure}[ht]
    ...
    \caption{GO-GARCH diagram representing function/method relationships in the package.}
...
in \eqref{eq:app2} provides for a method of moments type estimator. The shape parameter $\nu$ is then estimated keeping 
...
of the weighted moments generated by the model.\footnote{For the DCC and Copula DCC models only the first 2 moments
